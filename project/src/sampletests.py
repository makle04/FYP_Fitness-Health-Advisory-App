...
def get_recommendation(cluster):
    ...
    # ✅ Pull recommendation directly from dataset
    if "recommendation" in df.columns:
        return cluster_data["recommendation"].mode()[0]

    # fallback (just in case)
    return "Recommendation column not found in dataset."
...
for name, age, weight, height, session, calories, max_bpm, resting_bpm, avg_bpm, water in tests:
    ...
    cluster, recommendation = predict_cluster(
        age, weight, height, session, calories,
        max_bpm, resting_bpm, avg_bpm, water
    )
